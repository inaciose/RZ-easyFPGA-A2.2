...
BEGIN
	...
	entity work.PS2Kbd(rtl)
		port map(
			sys_clk      => clk,			-- base clock
			clk          => ps2_clk,	-- input ps2 clock from kb pin
			data         => ps2_data,  -- input ps2 data from kb pin
			code         => ps2_code,	-- output keycode
			has_new_code => ps2_code_new --output announcing that a valid key is pressed
		);	

  key_new <= ps2_code_new;
  key_code <= ps2_code;
